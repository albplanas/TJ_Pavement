...
class Profile extends Component {
  ...
  componentWillMount(){
    ...
    if (online){
      ...
      var newEmployeeList,newProjectList,newSupervisorList,newLaborList;
      //fetch data

      /*
      
              http://jva-sql:8080/Assistance/GetJson.php?table=?
              SyntaxEditor Code Snippet 
              /	Employee -> Listado de empleados /
                Supervisor -> Listado de supervisores /
                Project -> Listado de proyectos /	
                Labor -> Listado de Labor
     ...
     axios.get("http://jva-sql:8080/Assistance/GetJson.php?table=Employee")
    .then((response)=> {
      newEmployeeList=response.data
      TjStorage.IdEmployee =response.data.map(elem=>{return [elem.Employee,elem.IdEmployee]})
    ...
    axios.get("http://jva-sql:8080/Assistance/GetJson.php?table=Project")
    .then((response)=> {   TjStorage.Projects =response.data.map(elem=>{return [elem.projectcode,elem.idProject,elem.projectname]})})
    .catch(error => {
      console.log("this is the error",error)
    })

    axios.get("http://jva-sql:8080/Assistance/GetJson.php?table=supervisor")
    .then((response)=> {
      newSupervisorList=response.data
    })
    .catch(error => {
      console.log("this is the error",error)
    })
    ...
    axios.get("http://jva-sql:8080/Assistance/GetJson.php?table=Labor")
    .then((response)=> {
      TjStorage.IdLabor =response.data.map(elem=>{return [elem.idLabor,elem.labor] })
      TjStorage.IdCategories=response.data.map(elem=>{return [elem.idLabor,elem.labor] })
    })
    .catch(error => {
      console.log("this is the error",error)
    })
    ...
    render() { 
     ...
     console.log("this.props.getReport",this.props.getReport)
